...
### 1. Start the IKEv2 VPN Server
    ...
    docker run --privileged -d --name ikev2-vpn-server2 --restart=always --network host lifeibo/docker-ikev2-vpn-server:latest

### 2. Generate the .mobileconfig (for iOS / macOS)

    docker run --privileged -i -t --rm --volumes-from ikev2-vpn-server2 -e "HOST=vpn1.example.com" lifeibo/docker-ikev2-vpn-server:latest generate-mobileconfig > ikev2-vpn.mobileconfig
